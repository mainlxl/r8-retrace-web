...
func indexFile(w http.ResponseWriter, r *http.Request) {
	file, err := os.ReadFile("input.html")
	if err != nil {
		w.Write([]byte(err.Error()))
		return
	}
...
func index(w http.ResponseWriter, r *http.Request) {
	//使用input.html压缩(https://c.runoob.com/front-end/47/)
	html := `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>R8-retrace</title>
</head>
...
<script>
    ...
    function retrace() {
        ...
        if (log === '') {
            Toast("请输入要retrace的日志")
            return
        }
        let loading = Loading("解析中....");
        axios.post('/retrace', {
            mapping: mapping,
            log: log,
    ...
    function Toast(msg, duration, isAutoClose = true) {
    ...
    function Loading(msg) {
        return Toast(msg, 0, false)
    }

    function removeLoading(m) {
        document.body.removeChild(m)
    }
